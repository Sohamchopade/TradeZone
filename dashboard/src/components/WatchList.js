...
useEffect(() => {
  const fetchStockPrices = async () => {
    try {
      ...
      const updatedStocks = initialWatchlist.map((stock) => {
        const apiStock = apiStocks.find(
          (item) => item.symbol === stock.name
        );

        // If BharatStock doesn't find the stock,
        // keep the original data for now
        if (!apiStock || !apiStock.found) {
          return stock;
        }

        return {
          ...stock,
          price: apiStock.close,
          percent: `${apiStock.changePct >= 0 ? "+" : ""}${apiStock.changePct.toFixed(2)}%`,
          isDown: apiStock.changePct < 0,
        };
      });
